...
export function Nav() {
  return <header className="site-header"><div className="shell"><nav className="nav" aria-label="Primary navigation"><Link href="/" className="brand link" aria-label="BaseLens home"><BrandMark /><span>BaseLens</span></Link><div className="navlinks"><Link href="/analyze">Analyze</Link><Link href="/compare">Compare</Link><Link href="/how-it-works">How it works</Link><Link href="/methodology">Methodology</Link></div><div className="nav-tools"><Link href="/analyze" className="nav-action link">Launch agent <ArrowRightIcon size={14}/></Link><MobileMenu/></div></nav></div></header>;
}
