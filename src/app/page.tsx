    ...
    <main id="main-content" className="product-home">
      <section className="product-hero">
        ...
        <div className="shell product-hero-inner">
          ...
          <p>BaseLens researches any Base token contract, with extra depth for unfamiliar and fast-moving tokens. It checks the evidence, grades the profile, and explains what still needs your attention.</p>
          <div className="product-actions">
            <Link className="product-primary" href="/analyze">Analyze a token <span>→</span></Link>
            <Link className="product-secondary" href={`/analyze/${demoContract}`}>Open live demo</Link>
          </div>
      ...
      <section className="product-preview-section">
        <div className="shell product-preview-wrap">
          <div className="product-preview-bar"><span>Illustrative workspace · values intentionally omitted</span><b>Evidence first · Agent explained</b></div>
          <div className="product-preview">
            <aside>
              <div className="preview-token"><div>?</div><span><b>BASE TOKEN</b><small>Example layout</small></span></div>
              <nav><strong>Overview</strong><span>Market</span><span>Holders</span><span>Security</span><span>Social</span></nav>
              <div className="preview-status"><i /> Research workflow</div>
            </aside>
            <div className="preview-main">
              <header><div><small>BASELENS SCORE</small><strong>--<span>/100</span></strong><em>Calculated from evidence</em></div><div className="preview-agent"><small>AGENT VERDICT</small><p>The agent explains the strongest signal, biggest risk and what evidence is still missing.</p></div></header>
              <div className="preview-metrics"><div><span>Liquidity</span><b>--</b></div><div><span>Safety</span><b>--</b></div><div><span>Holders</span><b>--</b></div><div><span>Social</span><b>--</b></div></div>
              <div className="preview-evidence"><div><small>STRONGEST SIGNAL</small><b>Shown from collected evidence</b></div><div><small>BIGGEST RISK</small><b>Separated from missing data</b></div><div><small>CONFIDENCE</small><b>Based on evidence coverage</b></div></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="agent-loop-section">
        <div className="shell agent-loop-grid">
          ...
          <div className="agent-loop"><div><b>Observe</b><span>Collect evidence</span></div><i>→</i><div><b>Analyze</b><span>Normalize signals</span></div><i>→</i><div><b>Reason</b><span>Weigh trade-offs</span></div><i>→</i><div><b>Decide</b><span>Grade the profile</span></div><i>→</i><div><b>Explain</b><span>Show the evidence</span></div></div>
        </div>
      </section>
